...
The homepage body has one source: `content/research-home.md`. Theme headings use an explicit Markdown identifier such as `## Memory {#memory}`. The questions beneath each heading appear directly in the main body. These headings are not note links, and no example theme note pages are published. The sliding-note interface remains available for actual authored notes once a separate note source is added. External links in the homepage prose continue to open separately.

There is currently no browser editor for public research notes. Edit `content/research-home.md` in the repository to change the main-page research questions. This does **not** create a separate note. The published page also fetches the latest copy of this file from the repository on load; a Worker deployment updates the bundled fallback and initial HTML. `/login` and `/library` are only for private census annotations, not public notes.
